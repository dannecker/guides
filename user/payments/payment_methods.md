...
Payment methods represent the different payment options available to customers during the checkout process on an e-commerce store. Colibri supports many types of payment methods, including both online and offline options. This guide describes how to add payment methods to your Colibri store.
...
Colibri enables you to utilize the payment method of choice for your e-commerce store. We have two [preferred payment gateway partners](http://usoft.com.ua/colibri/products/payment_processing) and a long [list](https://github.com/Shopify/active_merchant#supported-direct-payment-gateways) of payment gateways that are supported by default in Colibri. We also enable you to add a custom payment gateway, as well as offer offline payment options such as checks and purchase orders.
...
To configure one of the supported payment gateways, you must first install the [Colibri_Gateway](https://github.com/colibri/colibri_gateway) extension on your store. More than likely, you will want to ask someone from your technical team to do this. Once this extension has been installed, you can configure one of the supported gateways in the Admin Interface by clicking the "Configuration" tab and then clicking the "New Payment Method" button.

![New Payment Method Form](/images/user/payments/new_payment_method.jpg)

If you installed the [Colibri_Gateway](https://github.com/colibri/colibri_gateway) extension, you will see a long list of gateways in the "Provider" drop down menu. Select the one that you would like to add.
...
* **Development** - Used by developers when they are testing a Colibri store on their local machine.
* **Production** - Select if you want the payment gateway to appear on the customer-facing area of your store.
* **Test** - Used by developers who are testing their Colibri store, typically with our [test suite](/developer/testing.html).
