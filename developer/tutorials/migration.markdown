---
title: Migrating to Colibri
section: advanced
---

This section explains how to convert existing sites or data sets for
use with Colibri. It is a mix of tips and information about the relevant
...
Colibri uses fixtures to load up the sample data. It's a convenient
...
Colibri has several options for loading seed data, but perhaps the easiest
...
A related but important topic is the Colibri core settings that your app
...
<% ruby do %>
        p = Colibri::Product.create :name => 'some product', :price => 10.0,
...
have configured Colibri's [inventory system](inventory.html), but most sites
...
The *Colibri::ShippingCategory* model is effectively a wrapper for a
...
<% ruby do %>
    p.shipping_category = Colibri::ShippingCategory.where(:name => 'Type
A').first_or_create
<% end %>
...
<% ruby do %>
    # create outside of loop
      main_taxonomy = Colibri::Taxonomy.where(:name => 'Products').first_or_create
    ...
    # inside of main loop
     ...
     taxon_col.split(/[\r\n]*/).each do |chain|
         ...
         names = chain.split
         names.each do |name|
            taxon = Colibri::Taxon.where.first_or_create
         end
         the_taxons << taxon
     end
     p.taxons = the_taxons
...
<% ruby do %>
    size_prop = Colibri::Property.where(name: 'size', presentation: 'Size').first_or_create
<% end %>
...
column, this means:
<% ruby do %>
    Colibri::ProductProperty.create :property => size_prop, :product =>
      p, :value => size_info
<% end %>
...
***
Colibri's core generally assumes that each variant has exactly one
...
<% ruby do %>
  v = Colibri::Variant.create :product => p, :sku => "some_sku_code", :price => NNNN
<% end %>
...
The exact steps depend on how you have configured Colibri's
...
<% ruby do %>
    p.option_types = option_names_col.map do |name|
      Colibri::OptionType.where(:name => name, :presentation => name).first_or_create
    end
<% end %>
...
<% ruby do %>
  *,opts,sku,price = opt_info.match\s*=\s*\s*@.\*?)/).to_a
  v = Colibri::Variant.create :product => p, :sku => sku, :price => price
  v.option_values = opts.split.map do |nm|
    Colibri::OptionValue.where.first_or_create
  end
<% end %>
...
increasing size or by alphabetical order. The *Colibri::OptionValue* model uses
...
<% ruby do %>
  color_type = Colibri::OptionType.create :name => 'Color', :presentation => 'Color'
  color_options = %w[Red Blue Green].split.map { |n|
    Colibri::OptionValue.create :name => n, :presentation => n,
...
optioning](http://blog.endpoint.com/2010/01/rails-ecommerce-colibri-hooks-comments.html).
...
Colibri uses [paperclip](https://github.com/thoughtbot/paperclip) to
...
<% ruby do %>
    ...
    img = Colibri::Image.create(:attachment => File.open(path), :viewable => product.master)

    #for image for single variant
    img = Colibri::Image.create(:attachment => File.open(path), :viewable => variant)
<% end %>
