...
By having a master variant, the code within Colibri to track  is simplified.
...
Colibri automatically handles creation and storage of several size versions of each image (via the Paperclip plugin). The default styles are as follows:
...
These sizes can be changed by altering the value of `Colibri::Config[:attachment_styles]`. Once `Colibri::Config[:attachment_styles]` has been changed, you *must* regenerate the paperclip thumbnails by running this command:

```bash
$ bundle exec rake paperclip:refresh:thumbnails CLASS=Colibri::Image
```

If you want to change the image that is displayed when a product has no image, simply create new versions of the files within [Colibri's app/assets/images/noimage directory](https://github.com/colibri/colibri/tree/master/frontend/app/assets/images/noimage). These image names must match the keys within `Colibri::Config[:attachment_styles]`.
...
You may see what price a product would be in the current currency (`Colibri::Config[:currency]`) by calling the `price` method on that instance:
...
Taxons use the [Nested set model](http://en.wikipedia.org/wiki/Nested_set_model) for their hierarchy. The `lft` and `rgt` columns in the `colibri_taxons` table represent the locations within the hierarchy of the item. This logic is handled by the [awesome_nested_set](https://github.com/collectiveidea/awesome_nested_set) gem.
...
Linking to a taxon in a controller or a template should be done using the `colibri.nested_taxons_path` helper, which will use the taxon's permalink to
