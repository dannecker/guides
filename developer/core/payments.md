...
Colibri has a highly flexible payments model which allows multiple payment methods to be available during checkout. The logic for processing payments is decoupled from orders, making it easy to define custom payment methods with their own processing logic.

Payment methods typically represent a payment gateway. Gateways will process card payments, and may also include non-gateway methods of payment such as Check, which is provided in Colibri by default.

The `Payment` model in Colibri tracks payments against [Orders](orders). Payments relate to a `source` which indicates how the payment was made, and a `PaymentMethod`, indicating the processor used for this payment.
...
The state transition for these is handled by the processing code within Colibri; however, you are able to call the event methods yourself to reach these states. The event methods are:
...
Payment methods represent the different options a customer has for making a payment. Most sites will accept credit card payments through a payment gateway, but there are other options. Colibri also comes with built-in support for a Check payment, which can be used to represent any offline payment. There are also third-party extensions that provide support for some other interesting options such as [better_colibri_paypal_express](https://github.com/colibri-contrib/better_colibri_paypal_express).

A `PaymentMethod` can have the following attributes:

* `type`: The subclass of `Colibri::PaymentMethod` this payment method represents. Uses rails single table inheritance feature.
...
By default, a payment method's `auto_capture?` method depends on the `Colibri::Config[:auto_capture]` preference. If you have set this preference to `true`, but don't want a payment method to be auto-capturable like other payment methods in your system, you can override the `auto_capture?` method in your
`PaymentMethod` subclass:

```ruby
class FancyPaymentMethod < Colibri::PaymentMethod
  def auto_capture?
    false
  end
end
...
Payment processing in Colibri supports many different gateways, but also attempts to comply with the API provided by the [active_merchant](https://github.com/shopify/active_merchant) gem where possible.
...
Colibri stores only the type, expiration date, name and last four digits for the card on your server. This data can then be used to present to the user so that they can verify that the correct card is being used. All credit card data sent through forms is sent through immediately to the gateways, and is not stored for any period of time.

### Processing Walkthrough

When an order is completed in colibri, each `Payment` object associated with the order has the `process!` method called on it (unless `payment_required?` for the order returns `false`), in order to attempt to automatically fulfill the payment required for the order. If the payment method requires a source, and the payment has a source associated with it, then Colibri will attempt to process the payment. Otherwise, the payment will need to be processed manually.
...
Responses from payment gateways within Colibri are typically `ActiveMerchant::Billing::Response` objects. When Colibri handles a response from a payment gateway, it will serialize the object as YAML and store it in the database as a log entry for a payment. These responses can be useful for debugging why a payment has failed.

You can get a list of these log entries by calling the `log_entries` on any `Colibri::Payment` object. To get the `Active::Merchant::Billing::Response` out of these `Colibri::LogEntry` objects, call the `details` method.

## Supported Gateways

Access to a number of payment gateways is handled with the usage of the [colibri_gateway](https://github.com/colibri/colibri_gateway) extension. This extension currently supports the following gateways:
...
With the `colibri_gateway` gem included in your application's `Gemfile`, these gateways will be selectable in the admin backend for payment methods.
...
In order to implement a new gateway in the colibri_gateway project, please refer to the other gateways within `app/models/colibri/gateway` inside that project.
***

## Adding your custom gateway

In order to make your custom gateway show up on backend list of available payment methods
you need to add it to colibri config list of payment methods first. That can be achieved
by adding the following code in your colibri.rb for example:

```ruby
Rails.application.config.colibri.payment_methods << YourCustomGateway
```

[better_colibri_paypal_express](https://github.com/colibri-contrib/better_colibri_paypal_express) and
[colibri-adyen](https://github.com/colibri/colibri-adyen) are good examples of standalone custom gateways.
No dependency on colibri_gateway or activemerchant required.
