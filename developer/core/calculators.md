...
Colibri makes extensive use of the `Colibri::Calculator` model and there are several subclasses provided to deal with various types of calculations (flat rate, percentage discount, sales tax, VAT, etc.) All calculators extend the `Colibri::Calculator` class and must provide the following methods:
...
Calculators link to a `calculable` object, which are typically one of `Colibri::ShippingMethod`, `Colibri::TaxRate`, or `Colibri::Promotion::Actions::CreateAdjustment`. These three classes use the [`Colibri::Core::CalculatedAdjustment`](#calculated-adjustments) module to provide an easy way to calculate adjustments for their objects.

## Available Calculators

The following are descriptions of the currently available calculators in Colibri. If you would like to add your own, please see the [Creating a New Calculator](#creating-a-new-calculator) section.
...
This calculator depends on its `calculable` responding to a `promotion` method, which should return a `Colibri::Promotion` (or similar) object. This object should then return a list of rules, which should respond to a `products` method. This is used to return a result of matching products.
...
* `currency`: The currency for this calculator. Defaults to the currency you have set for your store with `Colibri::Config[:currency]`
...
To create a new calculator for Colibri, you need to do two things. The first is to inherit from the `Colibri::Calculator` class and define `description` and `compute` methods on that class:

```ruby
class CustomCalculator < Colibri::Calculator
  def self.description
    # Human readable description of the calculator
  end
...
If you are creating a new calculator for shipping methods, please be aware that you need to inherit from `Colibri::ShippingCalculator` instead, and define a `compute_package` method:

```ruby
class CustomCalculator < Colibri::ShippingCalculator
  def self.description
    # Human readable description of the calculator
  end
...
The second thing is to register this calculator as a tax, shipping, or promotion adjustment calculator by calling code like this at the end of `config/initializers/colibri.rb` inside your application (`config` variable defined for brevity):

```ruby
config = Rails.application.config
config.colibri.calculators.tax_rates << CustomCalculator
config.colibri.calculators.shipping_methods << CustomCalculator
config.colibri.calculators.promotion_actions_create_adjustments << CustomCalculator
```
...
If you wish to use Colibri's calculator functionality for your own application, you can include the `Colibri::Core::CalculatedAdjustments` module into a model of your choosing.
...
```ruby
config.colibri.calculators.plans << CustomCalculator
```
