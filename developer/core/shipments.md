...
This guide explains how Colibri represents shipping options and how it calculates expected costs, and shows how you can configure the system with your own shipping methods. After reading it you should know:

* how shipments and shipping are implemented in Colibri
...
Colibri uses a very flexible and effective system to calculate shipping, accommodating the full range of shipment pricing: from simple flat rate to complex product-type- and weight-dependent calculations.

The Shipment model is used to track how items are delivered to the buyer.

Shipments have the following attributes:

* `number`: The unique identifier for this shipment. It begins with the letter H and ends in an 11-digit number. This number is shown to the users, and can be used to find the order by calling `Colibri::Shipment.find_by(number: number)`.
...
Explaining each piece of the shipment world inside of Colibri separately and how each piece fits together can be a cumbersome task. Fortunately, using a few simple examples makes it much easier to grasp. In that spirit, the examples are shown first in this guide.
...
Colibri ships with 5 default Calculators:
...
***
You can enable collection of extra _shipping instructions_ by setting the option `Colibri::Config.shipping_instructions` to `true`. This is set to `false` by default. See [Shipping Instructions](#shipping-instructions) below.
***
...
Colibri comes with a set of calculators that should fit most of the shipping situations that may arise. If the calculators that come with Colibri are not enough for your needs, you might want to use an extension - if one exists to meet your needs - or create a custom one.

### Extensions

There are a few Colibri extensions which provide additional shipping methods, including special support for fees imposed by common carriers, or support for bulk orders. See the [Colibri Extension Registry](http://usoft.com.ua/colibri/extensions) for the latest information.
...
The option `Colibri::Config[:shipping_instructions]` controls collection of additional shipping instructions. This is turned off (set to `false`) by default. If an order has any shipping instructions attached, they will be shown in an order's shipment admin page and can also be edited at that stage. Observe that instructions are currently attached to the _order_ and not to actual _shipments_.

## The Active Shipping Extension

The popular `colibri_active_shipping` extension harnesses the `active_shipping` gem to interface with carrier APIs such as USPS, Fedex and UPS, ultimately providing Colibri-compatible calculators for the different delivery services of those carriers.

To install the `colibri-active-shipping` extension add the following to your `Gemfile`:

```ruby
gem 'colibri_active_shipping'
gem 'active_shipping', :git => 'git://github.com/Shopify/active_shipping.git'
```

and run `bundle install` from the command line.

As an example of how to use the [colibri_active_shipping extension](https://github.com/colibri/colibri_active_shipping) we'll demonstrate how to configure it to work with the USPS API. The other carriers follow a very similar pattern.
...
The `colibri_active_shipping` extension comes with several pre-configured calculators out of the box. For example, here are the ones provided for the USPS carrier:
...
The `colibri_active_shipping` gem needs some configuration variables set in order to consume the carrier web services.

```ruby
  # these can be set in an initializer in your site extension
  Colibri::ActiveShipping::Config.set(:usps_login => "YOUR_USPS_LOGIN")
  Colibri::ActiveShipping::Config.set(:fedex_login => "YOUR_FEDEX_LOGIN")
  Colibri::ActiveShipping::Config.set(:fedex_password => "YOUR_FEDEX_PASSWORD")
  Colibri::ActiveShipping::Config.set(:fedex_account => "YOUR_FEDEX_ACCOUNT")
  Colibri::ActiveShipping::Config.set(:fedex_key => "YOUR_FEDEX_KEY")
```

### Adding Additional Calculators

Additional delivery services that are not pre-configured as a calculator in the `colibri_active_shipping` extension can be easily added. Say, for example, you need First Class International Parcels via the US Postal Service.
...
class Calculator::ActiveShipping < Calculator
  def compute(line_items)
    ...
    rate = rates[self.description].to_f + (Colibri::ActiveShipping::Config[:handling_fee].to_f || 0.0)
...
As you can see in the code above, the `colibri_active_shipping` gem returns an array of services with their corresponding prices, which the `retrieve_rates` method converts into a hash. Below is what would get returned for an order with an international destination:
...
Split shipments are a new feature as of Colibri 2.0 that addresses the needs of complex Colibri stores that require sophisticated shipping and warehouse logic. This includes detailed inventory management and allows for shipping from multiple locations.
...
`create_proposed_shipments` will initially call `Colibri::Stock::Coordinator.new(@order).packages`. This will return an array of packages. In order to determine which items belong in which package when they are being built, Colibri uses an object called a `Splitter`, described in more detail [below](#the-packer).
...
The `Colibri::Stock::Coordinator` is the starting point for determining shipments when calling `create_proposed_shipments` on an order. Its job is to go through each `StockLocation` available and determine what can be shipped from that location.

The `Colibri::Stock::Coordinator` will ultimately return an array of packages which can then be easily converted into shipments for an order by calling `to_shipment` on them.

### The Packer

A `Colibri::Stock::Packer` object is an important part of the `create_proposed_shipments` process. Its job is to determine possible packages for a given StockLocation and order. It uses rules defined in classes known as `Splitters` to determine what packages can be shipped from a `StockLocation`.
...
Colibri comes with two default splitters which are run in sequence. This means that the first splitter takes the packages array from the order, and each subsequent splitter takes the output of the splitter that came before it.
...
* **Weight Splitter**: Splits an order into packages based on a weight threshold. This means that each package has a mass weight. If a new item is added to the order and it causes a package to go over the weight threshold, a new package will be created so that all packages weigh less than the threshold. You can set the weight threshold by changing `Colibri::Stock::Splitter::Weight.threshold` (defaults to `150`) in an initializer.

#### Custom Splitters

Note that splitters can be customized, and creating your own can be done with relative ease. By inheriting from `Colibri::Stock::Splitter::Base`, you can create your own splitter.

For an example of a simple splitter, take a look at Colibri's [weight based splitter](https://github.com/colibri/colibri/blob/235e470b242225d7c75c7c4c4c033ee3d739bb36/core/app/models/colibri/stock/splitter/weight.rb). This splitter pulls items with a weight greater than 150 into their own shipment.

After creating your splitter, you need to add it to the array of splitters Colibri
uses. To do this, add the following to your application's colibri initializer
`colibri.rb` file:

```ruby
Rails.application.config.colibri.stock_splitters << Colibri::Stock::Splitter::CustomSplitter
```

You can also completely override the splitters used in Colibri, rearrange them, etc.
To do this, add the following to your `colibri.rb` file:

```ruby
Rails.application.config.colibri.stock_splitters = [
  Colibri::Stock::Splitter::CustomSplitter,
  Colibri::Stock::Splitter::ShippingCategory
]
```

Or if you don't want to split packages just set the option above to an empty
array. e.g. a store with the following configuration in colibri.rb won't have any
package splitted.
...
If you want to add different splitters for each `StockLocation`, you need to decorate the `Colibri::Stock::Coordinator` class and override the `splitters` method.

### The Prioritizer

A `Colibri::Stock::Prioritizer` object will decide which `StockLocation` should ship which package from an order. The prioritizer will attempt to come up with the best shipping situation available to the user.

By default, the prioritizer will first select packages where the items are on hand. Then it will try to find packages where items are backordered. During this process, the `Colibri::Stock::Adjuster` is also used to ensure each package has the correct number of items.

The prioritizer is also a customization point. If you want to customize which packages should take priority for the order during this process, you can override the `sort_packages` method in `Colibri::Stock::Prioritizer`.
...
* Subclass the [Colibri::Stock::Adjuster](https://github.com/colibri/colibri/blob/a55db75bbebc40f5705fc3010d1e5a2190bde79b/core/app/models/colibri/stock/adjuster.rb) class and override the the `adjust` method to get the desired functionality.
* Decorate the `Colibri::Stock::Coordinator` and override the `prioritize_packages` method, passing in your custom adjuster class to the `Prioritizer` initializer. For example, if our adjuster was called `Colibri::Stock::CustomAdjuster`, we would do the following:

```ruby
Colibri::Stock::Coordinator.class_eval do
  def prioritize_packages(packages)
    prioritizer = Prioritizer.new(order, packages, Colibri::Stock::CustomAdjuster)
...
The `Colibri::Stock::Estimator` loops through the packages created by the packer in order to calculate and attach shipping rates to them. This information is then returned to the user so they can select shipments for their order and complete the checkout process.
