...
This guide explains how to customize the internal Colibri code to meet
your exact business requirements including:

-   Extending and overriding existing Colibri models and controllers
-   Changing the output from an existing Colibri controller action
-   Customizing the image handling functionality.

### Extending Classes

All of Colibri's business logic (models, controllers, helpers, etc) can
...
extension is to create a file within the relevant **app/models/colibri** or
**app/controllers/colibri** directory with the original class name with
**_decorator** appended.

**Adding a custom method to the Product model:**
app/models/colibri/product_decorator.rb
...
app/controllers/colibri/products_controller_decorator.rb
...
With the release of 0.60.0 Colibri now supports a new way of overriding or
...
(that all Colibri controllers are now using). It accepts a hash of options
...
-   **:format** - The format of the request, (i.e. :html, :json, :js,
    etc). All Colibri controllers have a class level **respond_to**
...
**app/controllers/colibri/products_controller_decorator.rb** file.
...
<% ruby do %>
    Colibri::Admin::ProductsController.class_eval do
...
the Image class. If you want to modify the default Colibri product and
