...
This guide will walk you through configuring and deploying your Colibri
...
* Operating system dependencies required for Ruby, Rails and Colibri
* Ruby 2.1.0
* Rails 4.0.2
* PostgreSQL
...
called "colibri", but you may choose to call it whatever you wish.
***

To set up this new user, run these commands on the server:

```bash
$ useradd -d /home/colibri -m -s /bin/bash colibri
$ passwd colibri
```
...
```bash
$ mkdir /home/colibri/.ssh
$ chown colibri:colibri /home/colibri/.ssh
$ chmod 700 /home/colibri/.ssh
```
...
Set the filename to be [your home directory]/.ssh/colibri_rsa.
...
```bash
$ scp ~/.ssh/colibri_rsa.pub colibri@[your server's address]:~/.ssh/authorized_keys
```
...
```bash
$ ssh colibri@[your server's address] -i [your home directory]/.ssh/colibri_rsa
```
...
After this, if you attempt to run `ssh colibri@localhost` from within the server
...
Once these dependencies are installed, switch back into the `colibri` user and
...
The next step is to put your Colibri application onto the server. To do this, you
...
Then, inside the directory for your Colibri app, run this command to set up a
...
system you're using. If you're using Colibri, there's a high chance that's going
...
deploying to your server. In this guide, we've used "colibri" so far, but you may
...
```ruby
set :deploy_to, "/home/colibri/#{application}"
```
...
from GitHub to your server into a directory such as `/home/colibri/[application's
...
`/home/colibri/[application]/shared/bundle`. This is so that every release of your
...
the necessary tables for your Colibri store.
...
server's deploy user account, which in this guide has been "colibri" so far. Yours
could be different. To set up this database, run this command as `root`:

```bash
$ sudo -u postgres createdb colibri
$ sudo -u postgres createuser colibri
```
...
the application, and so you should place it at `/home/colibri/[application's
...
if env == "production"
  ...
  working_directory "/home/colibri/[application's name]/current"

  # feel free to point this anywhere accessible on the filesystem user 'colibri'
  shared_path = "/home/colibri/[application's name]/shared"
...
content:
    ...
    server {
      ...
      root /home/colibri/[your application's name]/current/public;
      ...
      location ~ ^/(system|assets|colibri)/  {
        root /home/colibri/[application's name]/current/public;
        expires max;
        break;
      }
    }
...
separate paths from the `public` directory: `system`, `assets` and `colibri`.

The `system` directory is where Paperclip typically would store assets. Colibri's
assets are located separately, under `colibri`.
...
Now with the database and web servers set up for your Colibri store correctly, the
...
If you have `colibri_auth_devise` installed, this command will also prompt you for
a username and password for your admin user. If you're not using
`colibri_auth_devise`, then you will need to set up a new user account manually in
the console and assign it the admin role, like this:

```ruby
user = User.create!(:email => "email@example.com", :password => "topsekret")
user.colibri_roles << Colibri::Role.find_by_name("admin")
user.save!
```
...
subsequent deploys they don't disappear. To do this, you can create a new `colibri`
directory within the `shared` directory by using this command:

```bash
mkdir -p /home/colibri/[application's name]/shared/colibri
```
...
namespace :images do
  task :symlink, :except => { :no_release => true } do
    run "rm -rf #{release_path}/public/colibri"
    run "ln -nfs #{shared_path}/colibri #{release_path}/public/colibri"
  end
end
after "bundle:install", "images:symlink"
```

Now upon every deploy, Capistrano will symlink the `colibri` directory in the `shared`
